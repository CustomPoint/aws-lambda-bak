...
yum install -y mongodb-org-tools"""
    ...
    for index, env in enumerate(envs):
        ...
        file_name = "db-backup-" + env + "-$(date +%d-%m-%Y_%H-%M).tar.gz"
        backups += "mkdir -p " + path + "\n"

        backups += "mongodump --out " + path + " --host $DB_HOST_IP" + str(index) + ":$DB_HOST_PORT" + str(index) + \
            " $DB_USER{0} $DB_PASS{0}".format(str(index)) + "\n"

        backups += "tar czf " + file_name + " " + path + "\n"
        backups += "aws s3 cp /tmp/db-backup-" + env + "*.tar.gz $S3_PATH" + "\n"
